...
public:
  ...
  bool operator()(const T* const state1, const T* const state2, const T* const state3, T* residual) const {
	...
	T d1_ = sqrt( pow(state1[0]-state2[0],2) + pow(state1[1]-state2[1],2) + pow(state1[2]-state2[2],2) );
	T d2_ = sqrt( pow(state2[0]-state3[0],2) + pow(state2[1]-state3[1],2) + pow(state2[2]-state3[2],2) );
	T v1_ = d1_ / (state2[3]);
	T v2_ = d2_ / (state3[3]);
  T a_ = (v2_ - v1_)/(state2[3] + state3[3]);
