    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const 
    {
        ...
        if (jacobians != NULL && jacobians[0] != NULL) 
        {
            if (d < 0.0001 || dp < 0.0001){ 
                ...
                jacobians[0][0] = 0.0;
                jacobians[0][1] = 2.0*wf_*((2.0*dp_x - 2.0*x)/(2.0*dp) - (2*fpr.x() - 2*x)/(2*d))*(df - dp + d);
                jacobians[0][2] = 2.0*wf_*((2.0*dp_y - 2.0*y)/(2.0*dp) - (2*fpr.y() - 2*y)/(2*d))*(df - dp + d);
                jacobians[0][3] = 2.0*wf_*((2.0*dp_z - 2.0*z)/(2.0*dp) - (2*fpr.z() - 2*z)/(2*d))*(df - dp + d);
            }
