...
public:
    ...
    struct ComputeDistanceObstaclesUAV 
    {
        ...
        bool operator()(const double *state1, double *dist_) const 
        {
			...
			double d_obs;
            if(use_dist_func){
				bool is_into_ = g_3D->isIntoMap((double)state1[1],(double)state1[2],(double)state1[3]);
				if (is_into_){
					TrilinearParams d = g_3D->getPointDistInterpolation((double)state1[1], (double)state1[2], (double)state1[3]);
					double x_ = state1[1];
					double y_ = state1[2];
					double z_ = state1[3];
