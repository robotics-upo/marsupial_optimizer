...
public:
					...
					const T* const statePos6, T* residual) const 
	{
		...
		T dist_1_ = sqrt(pow(statePos2[1]-statePos1[1],2) + pow(statePos2[2]-statePos1[2],2) + pow(statePos2[3]-statePos1[3],2));
		T dist_2_ = sqrt(pow(statePos3[1]-statePos2[1],2) + pow(statePos3[2]-statePos2[2],2) + pow(statePos3[3]-statePos2[3],2));
		T dist_3_ = sqrt(pow(statePos4[1]-statePos3[1],2) + pow(statePos4[2]-statePos3[2],2) + pow(statePos4[3]-statePos3[3],2));
		T dist_4_ = sqrt(pow(statePos5[1]-statePos4[1],2) + pow(statePos5[2]-statePos4[2],2) + pow(statePos5[3]-statePos4[3],2));
		T dist_5_ = sqrt(pow(statePos6[1]-statePos5[1],2) + pow(statePos6[2]-statePos5[2],2) + pow(statePos6[3]-statePos5[3],2));
		...
		if (dist_ < d_ref_)
			...
			residual[0] =  wf_ * 10.0 * (exp(dist_ - d_ref_)-1.0);
