 ...
 public:
                        ...
                        double** jacobians) const 
    {
		...
		if(g_3D->isIntoMap(x, y, z))
        {
			...
			if (dist_ < sb)
				residuals[0] = (1.0/dist_)*C;
			else
				residuals[0] = (1.0/dist_)*1.0;	
            if (jacobians != NULL && jacobians[0] != NULL) 
            {
                jacobians[0][0] = -C*(p.a1 + p.a4*y + p.a5*z + p.a7*y*z)/pow(p.a0 + p.a1*x + p.a2*y + p.a3*z + p.a4*x*y + p.a5*x*z + p.a6*y*z + p.a7*x*y*z,2);
                jacobians[0][1] = -C*(p.a2 + p.a4*x + p.a6*z + p.a7*x*z)/pow(p.a0 + p.a1*x + p.a2*y + p.a3*z + p.a4*x*y + p.a5*x*z + p.a6*y*z + p.a7*x*y*z,2);
                jacobians[0][2] = -C*(p.a3 + p.a5*x + p.a6*y + p.a7*x*y)/pow(p.a0 + p.a1*x + p.a2*y + p.a3*z + p.a4*x*y + p.a5*x*z + p.a6*y*z + p.a7*x*y*z,2);
            }
        }
        else
        {
